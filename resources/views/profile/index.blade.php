<x-app-layout>
    ...
    <div class="py-6 bg-gray-100 min-h-screen text-gray-900">
                ...
                <div class="px-8 pb-6 relative">
                    <div class="flex flex-col md:flex-row items-start md:items-end -mt-16 mb-4 gap-6">
                        ...
                        <div class="relative group">
                            ...
                            @if (Auth::id() === $user->id)
                                <form action="{{ route('profile.updateImages') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <label for="avatar_upload" class="absolute bottom-2 right-2 bg-gray-100 hover:bg-gray-200 text-gray-700 p-2 rounded-full border border-gray-300 z-20 transition cursor-pointer shadow-sm">
                        ...
                        <div class="flex gap-3 md:pb-6 w-full md:w-auto justify-center">
                            @if (Auth::id() === $user->id)
                                {{-- Tombol Edit Profile --}}
                                <a href="{{ route('profile.edit') }}" class="bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-700 px-4 py-2 rounded-md font-semibold transition flex items-center gap-2 shadow-sm">
                    ...
                    <nav class="flex space-x-2 justify-center md:justify-start overflow-x-auto">
                        @foreach(['Posts', 'About', 'Friends', 'Photos'] as $tab)
                            <a href="#" class="px-4 py-3 font-semibold text-sm rounded-md transition whitespace-nowrap {{ $tab === 'Posts' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-100' }}">
                                {{ $tab }}
                            </a>
                        @endforeach
                    </nav>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                    <div class="bg-white rounded-xl p-4 shadow border border-gray-200">
                        <h3 class="font-bold text-lg text-gray-900 mb-3">Intro</h3>
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    {{-- Create Post Trigger --}}
                    @if (Auth::id() === $user->id)
                        <div class="bg-white p-4 rounded-xl shadow border border-gray-200 flex items-center gap-3">
                            <img src="{{ Auth::user()->avatar ? asset('storage/'.Auth::user()->avatar) : 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name) }}" class="w-10 h-10 rounded-full object-cover border border-gray-200">
                            {{-- Input dibuat abu-abu terang --}}
                            <a href="{{ route('dashboard') }}" class="flex-1 bg-gray-100 border border-transparent hover:border-gray-300 rounded-full px-4 py-2 text-gray-500 hover:bg-gray-200 transition text-left text-sm">
                    ...
                    @forelse($user->posts()->latest()->get() as $post)
                        <div class="bg-white rounded-xl shadow border border-gray-200 p-4">
                            ...
                            <div class="flex items-center space-x-3 mb-3">
                                <img src="{{ $user->avatar ? asset('storage/'.$user->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($user->name) }}" class="w-10 h-10 rounded-full object-cover border border-gray-200">
                                <div>
                            ...
                            {{-- Body Post --}}
                            <div class="text-gray-800 text-sm mb-3 whitespace-pre-line">
                                {!! $post->body !!}
                            </div>

                            {{-- Attachments --}}
                            @if ($post->attachments->count() > 0)
                                <div class="grid grid-cols-2 gap-1 rounded-lg overflow-hidden border border-gray-100">
                    ...
                    @empty
                        <div class="bg-white p-10 rounded-xl shadow border border-gray-200 text-center">
